...
######################################### JAVA PROGRAM  #####################################################
def javaprogram():
        print "rqFolder : " + rqFolder
        print "folder : " + folder
        print "submissionOriFile : " + submissionOriFile
	Javafile = os.path.abspath(os.path.join(rqFolder, folder, submissionOriFile)) #creating a .java file
	print Javafile
	shutil.copyfile(submissionpath,Javafile)					 #copying into the java file
        print Javafile
	subprocess.call(["javac",Javafile])							 #Compiling the Java file
	classfile = submissionOriFile[:-5] + ".class"
	classname = submissionOriFile[:-5]
...
def RunNow(ProblemID, SubmissionID):
	...
	print "Success point 1"
        ...
        global sampleipfile
        global resultopfile
        global submissionOriFile
        submissionOriFile = submission.originalname
        print submissionOriFile
